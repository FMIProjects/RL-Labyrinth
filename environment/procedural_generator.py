...
def generate_maze(height=10, width=10):
    ...
    while the_hunt_is_on:
        ...
        while not (
            up_cell == 1 and down_cell == 1 and left_cell == 1 and right_cell == 1
        ):
...
def maze_scale_up(cell_neighbours):
    ...
    new_cell_neighbours = np.array(
        [[[0, 0, 0, 0] for _ in range(width * 2)] for _ in range(height * 2)]
    )
