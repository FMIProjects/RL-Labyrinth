...
class EnvRenderer:
    def __init__(self,maze_env: MazeEnv,cell_size = 20, fps = 30):
        ...
        if self.screen is None:
            ...
            self.screen = pygame.display.set_mode(
                (self.maze_env.width * self.cell_size, self.maze_env.height * self.cell_size)
            )
        ...
        self.agent_images = {
            self.maze_env.UP: pygame.transform.scale(pygame.image.load("assets/agent_up.png").convert_alpha(),
                                            (self.cell_size, self.cell_size)),
            self.maze_env.RIGHT: pygame.transform.scale(pygame.image.load("assets/agent_right.png").convert_alpha(),
                                               (self.cell_size, self.cell_size)),
            self.maze_env.DOWN: pygame.transform.scale(pygame.image.load("assets/agent_down.png").convert_alpha(),
                                              (self.cell_size, self.cell_size)),
            self.maze_env.LEFT: pygame.transform.scale(pygame.image.load("assets/agent_left.png").convert_alpha(),
                                              (self.cell_size, self.cell_size))
        }
    ...
    def render(self, mode="human"):
        ...
        for row in range(self.maze_env.height):
            for col in range(self.maze_env.width):
                ...
                if not down or row == self.maze_env.height - 1:
                    pygame.draw.line(self.screen, WHITE, (x, y + self.cell_size),
                                     (x + self.cell_size, y + self.cell_size), 3)
                # Draw left line
        ...
        self.screen.blit(agent_image, (self.maze_env.agent_pos[1] * self.cell_size, self.maze_env.agent_pos[0] * self.cell_size))

        # Draw the goal
        self.screen.blit(self.door_image, (self.maze_env.goal_pos[1] * self.cell_size, self.maze_env.goal_pos[0] * self.cell_size))
        ...
        if self.maze_env.agent_pos == self.maze_env.goal_pos:
            self.screen.blit(self.red_door_image,
                             (self.maze_env.goal_pos[1] * self.cell_size, self.maze_env.goal_pos[0] * self.cell_size))
